// frontend/src/pages/live-quiz/StudentArena.jsx
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-4">
        ...
        <div className="bg-slate-800 p-8 rounded-3xl text-center w-full max-w-md border border-slate-700 shadow-2xl mt-12">
          ...
          />
          <button onClick={joinRoom} className="w-full py-4 rounded-xl text-white font-bold text-lg bg-green-600 hover:bg-green-500 hover:scale-[1.02] shadow-lg shadow-green-600/30">
            Enter Room
          </button>
        </div>
      </div>
      ...
      {activeQ ? (
            ...
            {activeQ.options.map((opt, i) => (
              <button
                ...
                className={`p-6 rounded-2xl text-xl font-bold transition-all transform 
                  ...
                  ${!submitted ? (
                    i === 0 ? 'bg-rose-500 shadow-[0_8px_0_rgb(159,18,57)] hover:bg-rose-400 hover:-translate-y-1' : 
                    i === 1 ? 'bg-emerald-500 shadow-[0_8px_0_rgb(6,95,70)] hover:bg-emerald-400 hover:-translate-y-1' : 
                    i === 2 ? 'bg-blue-500 shadow-[0_8px_0_rgb(30,58,138)] hover:bg-blue-400 hover:-translate-y-1' : 
        ...
        <div className="w-full max-w-md mt-12 animate-fade-in">
          <div className="bg-slate-800 p-8 rounded-3xl border border-slate-700 shadow-2xl text-center mb-8">
            {selectedOption === null ? (
              <div><div className="text-6xl mb-4">⏳</div><h1 className="text-3xl font-black text-slate-400 mb-2">You Missed!</h1><p className="text-slate-500">You ran out of time.</p></div>
            ) : selectedOption === resultData.correctIndex ? (
              <div><div className="text-6xl mb-4">🎉</div><h1 className="text-3xl font-black text-emerald-400 mb-2">Correct!</h1><p className="text-slate-400">Awesome job!</p></div>
            ) : (
              <div><div className="text-6xl mb-4">❌</div><h1 className="text-3xl font-black text-rose-500 mb-2">Wrong!</h1><p className="text-slate-400">Better luck next time.</p></div>
            )}
          ...
          <div className="bg-slate-800 p-6 rounded-3xl border border-slate-700 shadow-xl">
            ...
            <ul className="space-y-3">
              {resultData.leaderboard.map((p, i) => (
                <li key={i} className={`flex justify-between p-3 rounded-xl ${p.name === (userData?.name || "Guest") ? 'bg-slate-700 border border-slate-500' : 'bg-slate-900 border border-slate-700/50'}`}>
                  <span className="font-bold text-slate-200"><span className="text-slate-500 mr-2">#{i+1}</span>{p.name} {p.name === (userData?.name || "Guest") && "(You)"}</span>
