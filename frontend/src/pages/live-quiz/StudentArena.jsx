...
const StudentArena = () => {
  ...
  // 🔥 THE FIX: Strict Join Function 🔥
  const joinRoom = () => {
      if(!inputCode) return alert("Please enter a room code first.");
      
      const cleanCode = inputCode.trim();

      if (!socket || !socket.connected) {
          alert(`CRITICAL ERROR: Your browser is not connected to the live server. \n\nServer URL: ${serverUrl}\nStatus: ${connectionStatus}`);
          return;
      }

      // We use a callback to force the server to acknowledge the click
      socket.emit("student_join_quiz", { roomCode: cleanCode, name: userData?.name || "Guest" }, (response) => {
          if (response && response.status === "error") {
              alert(`Host Server Rejected You: ${response.message}`);
          } else if (response && response.status === "success") {
              setJoined(true);
          } else {
              alert("Server did not respond. Check your backend deployment.");
          }
      });
  };
  ...
  if (!joined) {
    return (
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-4">
        ...
        {/* SERVER STATUS BADGE */}
        <div className="absolute top-8 bg-slate-800 border border-slate-700 px-4 py-2 rounded-full text-xs font-mono font-bold text-slate-300">
             Server: {serverUrl} | Status: <span className={connectionStatus.includes("🟢") ? "text-green-400" : "text-rose-400"}>{connectionStatus}</span>
        </div>
        ...
        <div className="bg-slate-800 p-8 rounded-3xl text-center w-full max-w-md border border-slate-700 shadow-2xl mt-12">
          ...
          <button 
            onClick={joinRoom} 
            className="w-full py-4 rounded-xl text-white font-bold text-lg transition-all shadow-lg bg-green-600 hover:bg-green-500 hover:scale-[1.02] shadow-green-600/30"
          >
            Enter Room
          </button>
        </div>
      ...
      {activeQ ? (
             ...
             {activeQ.options.map((opt, i) => (
                 <button 
                    ...
                    className={`p-6 rounded-2xl text-xl font-bold transition-all transform 
                        ...
                        ${!submitted ? (
                          i === 0 ? 'bg-rose-500 shadow-[0_8px_0_rgb(159,18,57)]' : 
                          i === 1 ? 'bg-emerald-500 shadow-[0_8px_0_rgb(6,95,70)]' : 
                          i === 2 ? 'bg-blue-500 shadow-[0_8px_0_rgb(30,58,138)]' : 
                                    'bg-amber-500 shadow-[0_8px_0_rgb(146,64,14)]'
        ...
        <div className="w-full max-w-md mt-12 animate-fade-in">
           <div className="bg-slate-800 p-8 rounded-3xl border border-slate-700 shadow-2xl text-center mb-8">
               {selectedOption === null ? (
                   <div><div className="text-6xl mb-4">⏳</div><h1 className="text-3xl font-black text-slate-400 mb-2">You Missed!</h1></div>
               ) : selectedOption === resultData.correctIndex ? (
                   <div><div className="text-6xl mb-4">🎉</div><h1 className="text-3xl font-black text-emerald-400 mb-2">Correct!</h1></div>
               ) : (
                   <div><div className="text-6xl mb-4">❌</div><h1 className="text-3xl font-black text-rose-500 mb-2">Wrong!</h1></div>
               )}
           ...
           <div className="bg-slate-800 p-6 rounded-3xl border border-slate-700 shadow-xl">
               ...
               <ul className="space-y-3">
                 {resultData.leaderboard.map((p, i) => (
                   <li key={i} className={`flex justify-between p-3 rounded-xl bg-slate-900 border border-slate-700/50`}>
                     <span className="font-bold text-slate-200"><span className="text-slate-500 mr-2">#{i + 1}</span>{p.name}</span>
